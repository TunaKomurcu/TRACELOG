...
import blocks, services, app as app_module, webhook_handler as webhook_module

bot_module = app_module
...
class TestBotCommands:
    ...
    def test_status_command(self):
        ...
        with patch.object(bot_module.httpx, "get", side_effect=side_effects):
            ...
            bot_module.handle_tracelog_command(ack, {"text": "status", "channel_id": "C123"}, say)
    ...
    def test_run_command_async(self):
        ...
        with (
            patch.object(bot_module.app.client, "chat_postMessage", return_value={"ts": "123"}) as post_mock,
            ...
            bot_module.handle_tracelog_command(ack, command, say)
    ...
    def test_logs_no_session(self):
        ...
        with patch.object(bot_module, "_get_json", return_value={"ok": True, "data": [{"session_id": "s1", "last_event_at": "2025-01-01T00:00:00Z"}]}) as get_json:
            ...
            bot_module.handle_tracelog_command(ack, {"text": "logs", "channel_id": "C123"}, say)
    ...
    def test_alerts_empty(self):
        ...
        with patch.object(bot_module, "_get_json", return_value={"ok": True, "data": []}):
            ...
            bot_module.handle_tracelog_command(ack, {"text": "alerts", "channel_id": "C123"}, say)
