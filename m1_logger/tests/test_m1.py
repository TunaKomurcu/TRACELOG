...
def find_latest_log():
    if os.name == "nt":
        log_dir = pathlib.Path(os.environ.get("TEMP", r"C:\\Temp")) / "sazabi"
    else:
        log_dir = pathlib.Path("/tmp") / "sazabi"
...
class TestOutputPassthrough:
    def test_stdout_passthrough(self):
        result = run_logger(sys.executable, "-c", "print(chr(104)+chr(101)+chr(108)+chr(108)+chr(111)+chr(32)+chr(115)+chr(97)+chr(122)+chr(97)+chr(98)+chr(105))")
        combined = result.stdout + result.stderr
        assert "hello sazabi" in combined
